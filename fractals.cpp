...
class CSquaresasao {
public:
  void afisare(double xmin, double ymin, double xmax, double ymax, int nivel){
    if(nivel==0){      
    }
    else{
      double lungime=(double)(xmax-xmin)/3;
      
      glBegin(GL_LINE_LOOP);
      glVertex2d(xmin+lungime,ymin+lungime);
      glVertex2d(xmin+lungime,ymax-lungime);
      glVertex2d(xmax-lungime,ymax-lungime);
      glVertex2d(xmax-lungime,ymin+lungime);
      glEnd();

      afisare(xmin,ymin,xmin+lungime,ymin+lungime,nivel-1);
      afisare(xmin,ymin+lungime,xmin+lungime,ymax-lungime,nivel-1);
      afisare(xmin+lungime,ymax-lungime,xmax-lungime,ymax,nivel-1);
      afisare(xmin,ymax-lungime,xmin+lungime,ymax,nivel-1);
      afisare(xmax - lungime, ymax - lungime, xmax, ymax, nivel - 1);
      afisare(xmax - lungime, ymin + lungime, xmax, ymax - lungime, nivel - 1);
      afisare(xmax - lungime, ymin, xmax, ymin + lungime, nivel - 1);
      afisare(xmin + lungime, ymin, xmax-lungime, ymin + lungime, nivel - 1);

    }
  }
};

class CArborePerronInv{
public:
  void arborePerron(double lungime, 
                    int nivel, 
                    double factordiviziune, 
...
class CSierpinsky {
public:
	void segmentSierpinsky(double lungime, int nivel, CPunct &p, CVector v,int polaritate)
	{
		CPunct p1;
		if (nivel == 0)
		{
		...
		{
			v.rotatie(polaritate *60);
			segmentSierpinsky(lungime / 2.0, nivel - 1, p, v,-polaritate);
			p1 = v.getDest(p, lungime / 2.0);

			v.rotatie(polaritate *-60);
			segmentSierpinsky(lungime / 2.0, nivel - 1, p1, v, polaritate);
	...
	void afisare(double lungime, int nivel)
	{
		CVector v1(10.0, 0.0);
		CPunct p1(-0.9, -0.9);
		segmentSierpinsky(lungime, nivel, p1, v1,1);
	}
};
...
public:
  ...
  void printeaza(double xmin, double ymin,double xmax, double ymax){
    double i,j,rap;
    int r;
    ...
    glTranslated((xmin + xmax) * 1 / (xmin - xmax), (ymin + ymax)  * 1 / (ymin - ymax), 0);
    glScaled(2.4 / (xmax - xmin), 2.4 / (ymax - ymin), 1);
    // afisarea propriu-zisa
    glBegin(GL_POINTS);
    
    for(i=xmin;i<=xmax;i+=RX_JF)
      for(j=ymin;j<=ymax;j+=RY_JF){
        CComplex z(i,j);
        r=isIn(z);
        if(r==0){
          ...
          rap=(double) -r/m.rmax;
          glColor3f(5*rap,1-5*rap,1-5*rap);
          glVertex2d(i,j);
        }
      }
    glEnd();
...
void Display7() {
  ...
  glBegin(GL_LINE_LOOP);
    ...
    glVertex2d(0.98, -0.98);
  glEnd();
  square.afisare(-0.9, -0.9, 0.9, 0.9, nivel+1);
  nivel++;
}
